...
export function getComponentConfiguration(
  ...
  if (!fs.existsSync(componentConfigurationPath)) {
    warning(`The component configuration file does not exist: ${componentConfigurationPath}`);
  ...
  try {
    ...
    warning(`Failed to parse the component configuration file: ${error?.message}`);
  ...
  if (!componentConfiguration.component || !componentConfiguration.component.trim()) {
    warning('The component name is missing from the component configuration file');
  ...
  if (componentConfiguration.deployment.count === undefined || isNaN(componentConfiguration.deployment.count)) {
    componentConfiguration.deployment.count = 1;
  }

  if (!componentConfiguration.deployment.job || !componentConfiguration.deployment.job.trim()) {
    componentConfiguration.deployment.job = componentName.split(':')[0];
  }

  if (componentConfiguration.deployment.type === undefined || !componentConfiguration.deployment.type.trim()) {
    componentConfiguration.deployment.type = 'service';
  }

  if (!['service', 'system'].includes(componentConfiguration.deployment.type)) {
    warning(`The deployment type for the component '${componentName}' is invalid`);
  ...
  if (componentConfiguration.deployment.count < 1) {
    warning(`The deployment count for the component '${componentName}' must be greater than 0`);

    return [false, undefined];
  }

  if (componentConfiguration.deployment.containers === undefined || componentConfiguration.deployment.containers.length === 0) {
  ...
  for (let i = 0; i < componentConfiguration.deployment.containers.length; i++) {
    ...
    if (container.resources !== undefined) {
      if ((container.resources.cpu === undefined || isNaN(container.resources.cpu)) && (container.resources.ram === undefined || isNaN(container.resources.ram))) {
        container.resources = undefined;
      }
    }

    if (container.network !== undefined) {
      if (container.network.mode === undefined || !container.network.mode.trim()) {
        container.network.mode = 'bridge';
      }

      if (container.network.mode !== 'bridge' && container.network.mode !== 'host' && container.network.mode !== 'none') {
      ...
      if (container.network.ports !== undefined) {
        ...
        for (const [, port] of container.network.ports) {
          ...
          if (port.static !== undefined && (port.static < 0 || port.static > 65535)) {
      ...
      for (const service of container.services) {
        ...
        if (service.checks !== undefined) {
          for (const check of service.checks) {
            ...
            if (check.port !== undefined && !container.network?.ports?.has(check.port)) {
    ...
    if (container.config_maps !== undefined) {
      for (const configMap of container.config_maps) {
        ...
        if (configMap.destination === undefined || !configMap.destination.trim()) {
          warning(`The config map destination for container ${i + 1} is missing`);
